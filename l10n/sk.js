OC.L10N.register(
    "integration_discourse",
    {
    "Discourse" : "Discourse",
    "Error during authentication exchanges" : "Chyba počas výmeny autentifikácie",
    "No API key returned by Discourse" : "Discurse nevrátil žiadny API kľúč",
    "Discourse read notifications" : "Discourse upozornenia na prečítanie.",
    "Discourse notifications" : "Discourse upozornenia",
    "Discourse topics and posts" : "Témy a príspevky Discourse",
    "Discourse posts" : "Discourse príspevky",
    "Discourse topics" : "Discourse témy",
    "posts" : "príspevky",
    "Bad HTTP method" : "Zlá metóda HTTP",
    "Bad credentials" : "Nesprávne prihlasovacie údaje",
    "Connected accounts" : "Prepojené účty",
    "Discourse integration" : "Discourse integrácia",
    "Integration of Discourse forum and mailing list management system" : "Integrácia s Discourse diskusným fórom a systémom správy mailinglistu",
    "Discourse integration provides a dashboard widget displaying your important notifications\n    and the ability to find topics and posts with Nextcloud's unified search." : "Integrácia s Discourse poskytuje miniaplikáciu pre dashboard ktorá zobrazuje vaše najdôležitejšie oznámenia.\n   a schopnosť nájsť témy a príspevky pomocou zjednoteného vyhľadávania Nextcloudu.",
    "For example, https://help.nextcloud.com" : "Napríklad,  https://help.nextcloud.com",
    "Successfully connected to Discourse!" : "Úspešne pripojené k Discourse!",
    "Discourse API-key could not be obtained:" : "Discourse kľúč k API sa nepodarilo získať:",
    "Nextcloud Discourse integration on {ncUrl}" : "Integrácia Nextcloud Discourse na {ncUrl}",
    "Discourse URL is invalid" : "Discourse URL je neplatná",
    "Discourse options saved" : "Nastavenia Discourse boli uložené",
    "Failed to save Discourse options" : "Nepodarilo sa uložiť nastavenia Discourse",
    "Failed to save Discourse nonce" : "Discourse nonce sa nepodarilo uložiť",
    "Protocol handler registration requires a secure context (https) or is not supported by your browser" : "Registrácia obsluhy protokolu vyžaduje zabezpečený kontext (https) alebo ho váš prehliadač nepodporuje",
    "If you fail getting access to your Discourse account, this is probably because your Discourse instance is not authorized to give API keys to your Nextcloud instance." : "Ak sa vám nepodarí získať prístup k svojmu účtu Discourse, je to pravdepodobne preto, že vaša inštancia Discourse nie je oprávnená poskytovať API kľúče vašej inštancii Nextcloud.",
    "Ask the Discourse admin to add this URI to the \"allowed_user_api_auth_redirects\" list in admin settings:" : "Požiadajte správcu Discourse, aby pridal tento identifikátor URI do zoznamu „allow_user_api_auth_redirects“ v nastaveniach správcu:",
    "Make sure you accepted the protocol registration on top of this page if you want to authenticate to Discourse." : "Ak sa chcete autentizovať v Discourse, uistite sa, že ste prijali registráciu protokolu v hornej časti tejto stránky.",
    "Use the button below to trigger the custom protocol handler registration. If no prompt appears, make sure it is not already registered." : "Pre spustenie registrácie vlastného obslužného programu protokolu použite tlačidlo nižšie. Ak sa nezobrazí žiadna výzva, uistite sa, že už nie je zaregistrovaný.",
    "Register protocol handler" : "Zaregistrovať obslužný program protokolu",
    "With Chrome/Chromium, you should see a popup on browser top-left to authorize this page to open \"web+nextclouddiscourse\" links." : "V prípade prehliadača Chrome/Chromium by sa v hornej ľavej časti tejto stránky mala zobraziť lišta, ktorá autorizuje túto stránku na otváranie odkazov „web + nextclouddiscourse.",
    "If you don't see the popup, you can still click on this icon in the address bar." : "Ak nevidíte vyskakovacie okno, stále môžete kliknúť na túto ikonu v paneli s adresou.",
    "Then authorize this page to open \"web+nextclouddiscourse\" links." : "Potom autorizujte túto stránku na otvorenie odkazov „web + nextclouddiscourse“.",
    "If you still don't manage to get the protocol registered, check your settings on this page:" : "Ak sa vám nepodarí protokol zaregistrovať, skontrolujte svoje nastavenia na tejto stránke:",
    "With Firefox, you should see a bar on top of this page to authorize this page to open \"web+nextclouddiscourse\" links." : "V prípade prehliadača Firefox by sa v hornej časti tejto stránky mala zobraziť lišta, ktorá autorizuje túto stránku na otváranie odkazov „web + nextclouddiscourse“.",
    "Enable navigation link" : "Povoliť navigačný odkaz",
    "Discourse instance address" : "Adresa inštancie Discourse",
    "Connect to Discourse" : "Pripojiť k Discourse",
    "Connected as {username}" : "Pripojený ako {username}",
    "Disconnect from Discourse" : "Odpojiť od Discourse",
    "Enable unified search for topics" : "Zapnúť jednotné vyhľadávanie tém",
    "Enable searching for posts" : "Povoliť vyhľadávanie príspevkov",
    "No Discourse account connected" : "Nie je pripojený žiadny Discourse účet",
    "Error connecting to Discourse" : "Chyba pri pripájaní k Discourse",
    "No Discourse notifications!" : "Žiadne upozornenia z Discourse!",
    "Failed to get Discourse notifications" : "Chyba pri získavaní upozornení z Discourse",
    "_{nb} item in your admins inbox_::_{nb} items in your admins inbox_" : ["{nb} položka vo vašej doručenej pošte pre administrátorov","{nb} položiek vo vašej doručenej pošte pre administrátorov","{nb} položiek vo vašej doručenej pošte pre administrátorov","{nb} položiek vo vašej doručenej pošte pre administrátorov"],
    "_{nb} item in your moderators inbox_::_{nb} items in your moderators inbox_" : ["{nb} položka vo vašej doručenej pošte pre moderátorov","{nb} položiek vo vašej doručenej pošte pre moderátorov","{nb} položiek vo vašej doručenej pošte pre moderátorov","{nb} položiek vo vašej doručenej pošte pre moderátorov"]
},
"nplurals=4; plural=(n % 1 == 0 && n == 1 ? 0 : n % 1 == 0 && n >= 2 && n <= 4 ? 1 : n % 1 != 0 ? 2: 3);");
